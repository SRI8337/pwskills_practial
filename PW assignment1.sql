...
-- Primary key: it is a field that can uniquely identify a row in a column. There can be only one primary key in a table and it cannot be a null value.
-- Foreign key: it is a field that refers to the primary key in another table. There can be more than one foreign key in a table.
show tables;
 describe actor;
